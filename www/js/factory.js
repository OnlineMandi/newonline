	...
	return {
		request: function(config) {
			...
			if(currenttime){
				...
				config.headers['X-Current-Time'] = 0;
			}
			...
			getOffers: function(product,grade) {
                ...
                for (var index in current_offer) {
                    if (!current_offer.hasOwnProperty(index)) {
                        ...
                        var val = weight.name;
                        var unit = units[product.unit].sname;
                        if (i == 0) {
                            i++;
                            continue;
                        }
                        else if(count - i == 1) {
                            rates.push({'name': val + unit + ' or above', 'value': current_offer[index], 'unit':  unit});
                        } else {
                            rates.push({'name':  'min ' + val + unit, 'value': current_offer[index], 'unit':  unit});
                        }
                    }
                    i++;
                }
...
.factory('CartFactory', ['$filter', '$http', 'AuthFactory', 'LSFactory', 'ProductsFactory', function($filter, $http, AuthFactory, LSFactory, ProductsFactory) {
        ...
        var CartAPI = {
            getwallet:function(){
                return $http.get(base + '/cart/get-wallet');
            },
		addtocart: function (product) {
			return $http.post(base + '/cart/addproduct', product);
		},
		placeOrder: function (p_method) {
			return $http.post(base + '/cart/place-order', {cart:LSFactory.get(cartKey),pm:p_method});
		},
