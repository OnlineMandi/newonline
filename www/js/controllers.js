...
.controller('HomeCtrl', ['$rootScope', '$scope', '$ionicPopup', 'Loader', 'AuthFactory', 'UserFactory', 'ProductsFactory',function($rootScope, $scope, $ionicPopup, Loader, AuthFactory, UserFactory, ProductsFactory) {
    ...
    .controller('CheckoutCtrl', ['$rootScope', 'Loader', '$scope', '$ionicHistory', 'CartFactory', 'AuthFactory', '$ionicPopover', '$location', 'UserFactory', '$filter', function($rootScope, Loader, $scope, $ionicHistory, CartFactory, AuthFactory, $ionicPopover, $location, UserFactory, $filter) {
        ...
        CartFactory.getwallet().success(function(data) {
            ...
            if($scope.wallet.status){
                $scope.limit = $scope.wallet.credit-$scope.wallet.debit + $scope.wallet.limit;
                if($scope.wallet.credit-$scope.wallet.debit + $scope.wallet.limit  >= $scope.cartDetails.gtotal){
                    ...
                    $scope.data.method = 1;
                    $scope.methods[1].msg = "Your cart total has exceeded the wallet credit limit of " + $filter('currency')($scope.limit, 'Rs.', 2);
                }
        ...
        $scope.getAlert = function(val){
            if($scope.haswallet && val==2 && $scope.creditlimit < $scope.cartDetails.gtotal){
                Loader.toggleLoadingWithMessage($scope.methods[1].msg);
            }
        }
